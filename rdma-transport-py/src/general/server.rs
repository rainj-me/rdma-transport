...
pub fn print_buffer(gpu_buffer: &GPUMemBuffer, notification: &Notification) {
    ...
    let device_buffer = GPUMemBuffer::new(gpu_buffer.get_base_ptr() + offset as u64, size as usize);
            ...
            rt.block_on(async {
                loop {
                    ...
                    tokio::select! {
                        ...
                        Ok(mut cm_id) = rdma::wait_request(&mut listen_id) => {
                            info!("start qp handshake");
                            rt.spawn( async move {
                                match rdma::accept(&mut cm_id, gpu_ordinal).await {
